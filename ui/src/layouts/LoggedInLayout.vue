    ...
    <!-- <v-app-bar @click.stop="mini = true" app clipped-right color="blue" dark>
      <v-toolbar-title :style="{ cursor: 'pointer' }" @click="goToHomePage">
        <v-row justify="center" align="center">
          ...
          <v-col>
            <span class=" bg-purple-700">River Adminz</span>
          </v-col>
        </v-row>
      </v-toolbar-title>
  ...
  <v-icon>mdi-logout</v-icon>
    ...
    <Dropdown class=" absolute top-8 right-8">
      ...
      <template v-slot:items>
        ...
        <span
          class="block px-4 py-2 text-sm capitalize leading-5 text-gray-700 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
          href="#" role="menuitem">{{ user_profile.username }}</span>
        <button @click="logout"
          class="block px-4 py-2 w-full text-left underline text-sm leading-5 !text-red-600 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
          role="menuitem">Log out</button>
      </template>
    </Dropdown>

    <v-content>
      <div align="start" justify="start" class="pa-0" fluid>
        <v-row no-gutters align="start" justify="start">
          <v-col xl="auto" lg="auto" md="auto" sm="auto" xs="auto">
            <div :class="mini ? 'mini-drawer-shadow' : 'drawer-shadow'" />
            <v-navigation-drawer v-model="drawer" :mini-variant="mini" permanent fixed class="drawer">
              <div>


                <v-list-item class="p-0" jusitfy="center" align="center">
                  <v-list-item-title>
                    <svg class="!max-w-[77px]" width="77" height="44" viewBox="0 0 77 44" fill="none"
...
export default {
  ...
  computed: {
    userInitials() {
      if (this.user_profile && this.user_profile.username) {
        const names = this.user_profile.username.split(' ');
...
.drawer {
  ...
  box-shadow: 0px 0px 64px 0px #D2D2D23D;
  padding: 40px 20px;
