<template>
  <v-container fluid>
    <v-row justify="center" align="center">
      <v-col class="flex justify-between items-center">
        ...
        <div class="" v-if="has_add_function_permission">
          <v-btn large color="primary" @click="goToCreateFunctionPage" dark class="rounded-[64px] !py-5 !px-4">
    ...
    <v-row justify="center" align="center">
      <v-col>
        <v-data-table :headers="headers" :items="items" :items-per-page="10" class="elevation-1 custom-data-table"
          :footer-props="{
          'items-per-page-text': 'View',
        }">
          <template v-slot:item="{ item }">

            <tr class="custom-row   group hover:!bg-[#fafafa] cursor-pointer border-transparent">
              <td
                class="text-[#121722] first-td  !border-b-transparent !border-t-transparent  !ps-[48px] overflow-hidden group-hover:text-[#5E45FF] group-hover:font-bold transition duration-150"
                @click="goToViewFunctionPage(item.id)">
                <div>
                  {{ item.name }}
                </div>
              </td>
              <td class="font-bold text-base text-[#A0A2A7]" @click="goToViewFunctionPage(item.id)">
                {{ item.version }}
              </td>
            ...
            item.date_updated | moment("DD/MM/YYYY ")
              ...
              <td class="">
                <div class="flex gap-4 items-center">
                  <div
                    class="flex items-center gap-2  border border-[#121722] px-4    py-2 rounded-[64px] ms-auto cursor-pointer"
                    @click="goToEditFunctionPage(item.id)">
                    <v-icon class="mr-1" color="#121722"
                      :disabled="!has_change_function_permission">mdi-pencil-outline</v-icon>
                    ...
                    >mdi-delete</v-icon
                  > -->
                    <span class="text-[#121722] font-bold text-base flex-1 text-center">Edit</span>
                  </div>
                  <div class="flex items-center gap-2 bg-[#FFDFDF] px-4 py-2 rounded-[64px] ms-auto cursor-pointer"
                    @click="showDeletingDialog(item)">
                    ...
                    <span class="text-[#FF0F0F] font-bold text-base flex-1 text-center">Delete</span>
                  </div>
                </div>
              </td>
            </tr>
    ...
    <v-dialog v-if="this.deletingFunction" v-model="deleteDialog" max-width="581">
      <v-card>
