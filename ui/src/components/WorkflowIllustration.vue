<template>
  ...
  >
    <span
      class="absolute top-0 left-0 px-6 py-4 rounded-md m-1 text-[#A0A2A7] bg-[#E9E9E9] z-10"
      >Diagram</span
    >
    <v-card-text>
      <v-container fluid>
            ...
            <svg id="svg" width="650" height="270" class="">
              <defs>
                <filter id="dropshadow" height="130%">
                  ...
                  <!-- Shadow offset -->
                  <feFlood flood-color="#5E45FF33" result="color" />
                  <!-- Change 'red' to your desired shadow color -->
                  ...
                  <feMerge>
                    ...
                    <!-- This contains the colored shadow -->
                    <feMergeNode in="SourceGraphic" />
                    <!-- This contains the original element -->
                  </feMerge>
                </filter>
                <marker
                  id="start-marker"
      ...
      <v-card :class="{ 'custom-dialoge': !canDelete }">
        <v-card-title class="headline flex flex-col gap-[17px]">
          ...
          <v-card-actions class="flex gap-[17px] items-center justify-start">
            ...
            >
              <span class="flex-1 text-base font-bold text-center "
                >No, keep</span
              >
            </div>
  ...
  methods: {
    ...
    drag(event) {
      ...
      if (
        ...
        let newLeft =
          parseInt(this.$refs.draggableContainer.style.left) + deltaX;
    ...
    onResize() {
      if (this.svg) {
        if (Math.abs(this.windowSize.x - window.innerWidth) > 50) {
          ...
          document.getElementById("svg").attributes.width.value =
            containerWidth - 24;
    ...
    _getChilds(stateId) {
      return this.transitions
        ...
        .map((transition) =>
          this._get_state_by_id(transition.destination_state_id)
        );
    },
    _initialize() {
      if (
        (this.states && this.states.length > 0) ||
        (this.transitions && this.transitions.length > 0)
      ) {
    ...
    _createEdge(transition) {
      ...
      this.graph.setEdge(
        ...
        transition.destination_state_id,
        {
          ...
          label: transition.name ? transition.name : transitionNameFallback,
          curve: d3.curveBasis,
        }
      );
    },
    ...
    _renderSketch() {
      if (this.states.length > 0 || this.transitions.length > 0) {
        ...
        inner.selectAll("g.node").each(function(nodeId, index) {
          if (index === 0) {
            // Apply 'root' class only to the first node
    ...
    _setEdgeLabelDefaultClass() {
      d3.selectAll(`g.edgeLabels > g.edgeLabel`).classed(
        "edge-label-UNSELECTED ",
        true
      );
    },
    _setEdgeOnclicks() {
      ...
      this.states.forEach((state) => {
        if (that.state_class_mapping && that.state_class_mapping[state.id]) {
          if (that.state_class_mapping[state.id].rect) {
            Object.keys(that.state_class_mapping[state.id].rect).forEach(
              (style) => {
                d3.select(`g#state_${state.id} rect`).style(
                  style,
                  that.state_class_mapping[state.id].rect[style]
                );
                d3.select(`g#state_${state.id}`).classed(
                  that.state_class_mapping[state.id].class,
                  true
                );
              }
            );
          ...
          if (that.state_class_mapping[state.id].label) {
            Object.keys(that.state_class_mapping[state.id].label).forEach(
              (style) => {
                d3.select(`g#state_${state.id} g.label`)
                  .style(
                    style,
                    `${that.state_class_mapping[state.id].label[style]}`
                  )
                  .classed(that.state_class_mapping[state.id].class, true);
              }
            );
          }
        }
    ...
    _getTransitionBy(sourceState, targetState) {
      return this.transitions.find(
        (transition) =>
          transition.source_state_id == sourceState.id &&
          transition.destination_state_id == targetState.id
      );
    },
    _selectEdge() {
      if (this.selected_transition_id) {
        var edge_elem = d3.select(
          `g#transition_${this.selected_transition_id}`
        );
        edge_elem
          .classed("edge-SELECTED", true)
          .classed("edge-UNSELECTED", false);

        var index = new Number(edge_elem.attr("index"));
        var edge_label_elem = d3
          .selectAll(`g.edgeLabels > g.edgeLabel`)
          .nodes()[index];
    ...
    _reCenterSketch() {
      var xCenterOffset =
        (this.svg.attr("width") - this.graph.graph().width) / 2;
    ...
    deleteState() {
      ...
      http.delete(
        `/state/delete/${workflow_id}/${this.stateToBeDeletedId}/`,
        () => {
          // this.fetchStates();
          this.$emit("refetch");
